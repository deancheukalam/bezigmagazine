...
jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					businessCapitalAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			businessCapitalGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				businessCapitalGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			businessCapitalGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	$('.customize-control-sortable-repeater-add').on( 'click', function(event) {
		event.preventDefault();
		businessCapitalAppendRow($(this).parent(), '');
		businessCapitalGetAllInputs($(this).parent());
	});
	...
	function businessCapitalAppendRow($element, defaultValue) {
	...
	function businessCapitalGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  businessCapitalGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function businessCapitalGetAllImageCheckboxes($element) {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].business-capitaltinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].business-capitaltinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].business-capitalmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['businessCapital-upsell'] = api.Section.extend( {
