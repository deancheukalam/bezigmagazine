/* global businessCapitalScreenReaderText */
/**
...
(function($) {
	...
	function initMainNavigation(container) {
		...
		var dropdownToggle = $('<button />', {
			...
			.append(businessCapitalScreenReaderText.icon)
			.append($('<span />', {
				'class': 'screen-reader-text',
				text: businessCapitalScreenReaderText.expand
			}));
		...
		container.find('.current-menu-ancestor > button')
			...
			.text(businessCapitalScreenReaderText.collapse);
		...
		container.find('.dropdown-toggle').on( 'click', function(e) {
			...
			// jscs:enable
			screenReaderSpan.text(screenReaderSpan.text() === businessCapitalScreenReaderText.expand ? businessCapitalScreenReaderText.collapse : businessCapitalScreenReaderText.expand);
		});
	}
	...
	(function() {
		...
		menuToggle.on('click.business-capital', function() {
	...
	(function() {
		...
		function toggleFocusClassTouchScreen() {
			if (window.innerWidth >= 910) {
				$(document.body).on('touchstart.business-capital', function(e) {
					if (!$(e.target).closest('.main-navigation li').length) {
						$('.main-navigation li').removeClass('focus');
					}
				});
				siteNavigation.find( '.menu-item-has-children > a, .page_item_has_children > a' ).on( 'touchstart.business-capital', function( e ) {
				...
				siteNavigation.find('.menu-item-has-children > a').off('touchstart.business-capital');
			}
		}

		if ('ontouchstart' in window) {
			$(window).on('resize.business-capital', toggleFocusClassTouchScreen);
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find('a').on('focus.business-capital blur.business-capital', function() {
	...
	$(document).ready(function() {
		$(window).on('load.business-capital resize.business-capital', function() {
			if (window.innerWidth < 910) {
