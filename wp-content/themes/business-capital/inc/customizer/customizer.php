 ...
 */
class Business_Capital_Customizer {
	...
	public function register_options( $wp_customize ) {
		...
		$section_visibility = Business_Capital_Customizer_Utilities::section_visibility();

		$section_visibility['excluding-home'] = esc_html__( 'Excluding Homepage', 'business-capital' );

		Business_Capital_Customizer_Utilities::register_option(
			array(
				'settings'          => 'business_capital_header_image_visibility',
				'type'              => 'select',
		...
		$wp_customize->add_section( new Business_Capital_Upsell_Section( $wp_customize, 'upsell_section',
			array(
				'title'           => esc_html__( 'Business Capital Pro Available', 'business-capital' ),
				'url'             => 'https://fireflythemes.com/themes/business-capital-pro',
	...
	public function preview_js() {
		...
		wp_enqueue_script( 'business-capital-customizer', get_template_directory_uri() . '/js/customizer-preview' . $min . '.js', array( 'customize-preview' ), business_capital_get_file_mod_date( '/js/customizer-preview' . $min . '.js' ), true );
	}

	/**
	 * Binds the JS listener to make Customizer business_capital_color_scheme control.
	 *
	 * @since 1.0
	 */
	public function customize_control_js() {
		...
		// Enqueue Select2.
		wp_enqueue_script( 'business-capital-select2-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/select2' . $min . '.js', array( 'jquery' ), '4.0.13', true );
		wp_enqueue_style( 'business-capital-select2-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/select2' . $min . '.css', array(), '4.0.13', 'all' );
		
		// Enqueue Custom JS and CSS.
		wp_enqueue_script( 'business-capital-custom-controls-js', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'js/customizer' . $min . '.js', array( 'jquery', 'jquery-ui-core', 'business-capital-select2-js' ), business_capital_get_file_mod_date( '/js/customizer' . $min . '.js' ), true );
		
		wp_enqueue_style( 'business-capital-custom-controls-css', trailingslashit( esc_url ( get_template_directory_uri() ) ) . 'css/customizer' . $min . '.css', null, business_capital_get_file_mod_date( '/css/customizer' . $min . '.css' ), 'all' );
 ...
 */
$business_capital_customizer = new Business_Capital_Customizer();
