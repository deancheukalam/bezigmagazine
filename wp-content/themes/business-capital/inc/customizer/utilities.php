...
class Business_Capital_Customizer_Utilities {
	/**
	...
	public static function register_option( $option ) {
		...
		$defaults = apply_filters( 'business_capital_customizer_defaults', array() );
		...
		if ( ! empty( $get_posts ) ) {
			foreach ( $get_posts as $get_post ) {
				...
				if ( empty( $title ) ) {
					...
					$title = sprintf( esc_html__( 'ID: %s No Title', 'business-capital' ), $get_post->ID );
				}
	...
	static function section_visibility() {
		$options = array(
			'disabled'     => esc_html__( 'Disabled', 'business-capital' ),
			'homepage'     => esc_html__( 'Homepage / Frontpage', 'business-capital' ),
			'entire-site'  => esc_html__( 'Entire Site', 'business-capital' ),
		);

		return apply_filters( 'business_capital_section_visibility_options', $options );
	}
}
 ...
 * Initialize class
 */
$business_capital_utilities = new Business_Capital_Customizer_Utilities();
