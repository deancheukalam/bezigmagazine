<?php
/**
 * Blossom Magazine Typography Related Functions
 *
 * @package Blossom_Magazine
 */

if( ! function_exists( 'blossom_magazine_fonts_url' ) ):
/**
 * 
*/ 
function blossom_magazine_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'blossom-magazine' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'blossom-magazine' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'blossom-magazine' );
...
if( ! function_exists( 'blossom_magazine_get_google_fonts' ) ) :
/**
 * Get Google Fonts
*/
function blossom_magazine_get_google_fonts(){
...
if( ! function_exists( 'blossom_magazine_get_websafe_font' ) ) :
/**
 * Function listing WebSafe Fonts and its attributes
*/
function blossom_magazine_get_websafe_font(){
    ...
    return apply_filters( 'blossom_magazine_standard_fonts', $standard_fonts );
}
endif;

if( ! function_exists( 'blossom_magazine_check_varient' ) ) :
/**
 * Checks for matched varients in google fonts for typography fields
*/
function blossom_magazine_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = blossom_magazine_get_google_fonts(); //Google Fonts
    $websafe_fonts = blossom_magazine_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function blossom_magazine_get_css_variant( $font_variant ){
    $v_array = array(
		'100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function blossom_magazine_is_google_font( $font ){
    $return = false;
    $websafe_fonts = blossom_magazine_get_websafe_font();
    if( $font ){
...
*/
function blossom_magazine_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = blossom_magazine_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( blossom_magazine_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = blossom_magazine_get_css_variant( blossom_magazine_check_varient( $font_family, $font_variant ) );
...
if( ! function_exists( 'blossom_magazine_get_all_fonts' ) ) :
/**
 * Return Web safe font and google font without variants
*/
function blossom_magazine_get_all_fonts(){
    $google = array();        
    $standard = apply_filters( 'blossom_magazine_standard_font', array(
		'georgia-serif'       => __( 'Georgia', 'blossom-magazine' ),
        'palatino-serif'      => __( 'Palatino Linotype, Book Antiqua, Palatino', 'blossom-magazine' ),
        'times-serif'         => __( 'Times New Roman, Times', 'blossom-magazine' ),
        'arial-helvetica'     => __( 'Arial, Helvetica', 'blossom-magazine' ),
        'arial-gadget'        => __( 'Arial Black, Gadget', 'blossom-magazine' ),
		'comic-cursive'       => __( 'Comic Sans MS, cursive', 'blossom-magazine' ),
		'impact-charcoal'     => __( 'Impact, Charcoal', 'blossom-magazine' ),
        'lucida'              => __( 'Lucida Sans Unicode, Lucida Grande', 'blossom-magazine' ),
        'tahoma-geneva'       => __( 'Tahoma, Geneva', 'blossom-magazine' ),
		'trebuchet-helvetica' => __( 'Trebuchet MS, Helvetica', 'blossom-magazine' ),
		'verdana-geneva'      => __( 'Verdana, Geneva', 'blossom-magazine' ),
        'courier'             => __( 'Courier New, Courier', 'blossom-magazine' ),
        'lucida-monaco'       => __( 'Lucida Console, Monaco', 'blossom-magazine' ),
	) );
